...
int main() {
    ...
    while (1) {
        ...
        if ((past_key == 0) && (input_data != 0)) {
            if (input_data & 0x01) {  // S1 pressed
                val = (val + 1) % 10000;  // Increment and display
            }
            if (input_data & 0x80) {  // S8 pressed
                val = 0;  // Reset the counter when S8 is pressed
            }
        }
        past_key = input_data;  // Store last state
				for (int i = 0; i < 100; i++)
            segment(val);
    }
}